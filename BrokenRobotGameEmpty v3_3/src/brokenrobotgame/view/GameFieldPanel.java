...
public class GameFieldPanel extends JPanel implements KeyListener {
    ...
    private static final Color RADIATION_COLOR = Color.BLACK;
    ...
    private void drawRadiation(Graphics g) {
        HashMap<CellPosition, Integer> radiationMap = new HashMap<>();
        addToGeneralRadiationMapRadiationClouds(radiationMap, _model.field().getBatteryPool());
        addToGeneralRadiationMapRadiationClouds(radiationMap, _model.field().getDoorPool());

        g.setColor(RADIATION_COLOR);
        for (var radiation : radiationMap.entrySet()) {
            Point lefTop = leftTopCell(radiation.getKey());
            g.drawString(String.valueOf(radiation.getValue())
                    , lefTop.x + CELL_SIZE / 3, lefTop.y + CELL_SIZE / 4 * 2 + FONT_HEIGHT);
        }
    }

    private void addToGeneralRadiationMapRadiationClouds(HashMap<CellPosition, Integer> generalMap, ArrayList objects) {
        for (var object : objects) {
            if (object instanceof Radioactiveable) {
                for (var radiation : ((Radioactiveable) object).getRadiationCloud().entrySet()) {
                    int value = generalMap.containsKey(radiation.getKey()) ? generalMap.get(radiation.getKey()) : 0;
                    generalMap.put(radiation.getKey()
                            , value + radiation.getValue());
                }
            }
        }
    }
    ...
    private void drawBattery(Graphics g, Battery battery, Point lefTop) {
        ...
        String str = "Б/" + battery.chargeCapacity() + "/" + battery.amountОfСharge();
        g.drawString(str, lefTop.x+CELL_SIZE/8, lefTop.y+CELL_SIZE/4+FONT_HEIGHT);
    ...
    private void drawDoor(Graphics g, Door door, Point lefTop) {
        ...
        if(direct.equals(Direction.west()) || direct.equals(Direction.east()))  // гор. дверь
        {
            ...
            if (door instanceof DamageableDoor)
                g.drawString(String.valueOf((((DamageableDoor) door).getHitPoints()))
                        , lefTop.x+CELL_SIZE/3, lefTop.y+CELL_SIZE/4+FONT_HEIGHT);
        }
        else                                                                    // верт. дверь
        {
            if(!door.isOpen())  // дверь закрытая
            ...
            if (door instanceof DamageableDoor)
                g.drawString(String.valueOf((((DamageableDoor) door).getHitPoints()))
                        , lefTop.x+CELL_SIZE/3, lefTop.x+CELL_SIZE/4+FONT_HEIGHT);
        }    
    ...
    public void keyPressed(KeyEvent ke) {
        ...
        else
        {
            if(ke.getKeyCode() == KeyEvent.VK_UP) {         // перемещаемся вверх
                ...
                if(battery != null)
                {
                    _model.robot().useBattery();
                }
            }
            else if(ke.getKeyCode() == KeyEvent.VK_Q && _model.robot() instanceof CanUseSkill) { // используем способность
                ((BatteryHunterRobot)_model.robot()).useSkill();
            }
        }
    }
